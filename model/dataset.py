...
class BaseDataset(object):
    ...
    def calculate_weight(self, hyperedge_index, num_nodes, num_edges):
        # 하이퍼엣지 매트릭스 생성
        rows, cols = hyperedge_index
        H = torch.zeros(num_nodes, num_edges, dtype=torch.float32)
        H[rows, cols] = 1
        ...
        # 하이퍼노드 중복 계산
        HtH = torch.matmul(Ht, H)
        
        overlab_hyperedge = HHt
        overlab_hypernode = HtH
                        
        return overlab_hyperedge, overlab_hypernode

    def find_2hop_neighbors(self, HHt, num_nodes):
        # 노드별 이웃 리스트 생성
        neighbors = [set(torch.where(HHt[i] > 0)[0].tolist()) for i in range(num_nodes)]
        # 최종 결과 행렬 생성
        overlab_hyperedge = HHt.clone()

        # 2-hop 관계 탐색
        n = HHt.size(0)
        for i in range(n):
            for j in range(n):
                if i != j and HHt[i, j] == 0:  # 직접 연결되지 않은 노드
                    # 간접 연결 (2-hop) 여부 확인
                    if len(neighbors[i].intersection(neighbors[j])) > 0:
                        overlab_hyperedge[i, j] = -1
        return overlab_hyperedge
